...
#+BEGIN_SRC python :session :results silent :async 
import pandas as pd
import numpy as np
...
:   File "/tmp/python-ivFuzd", line 1, in <module>
:     raw_dataframe = pd.read_csv('train.csv')
...
rf = RandomForestClassifier(n_estimators=10, max_depth=15, max_features="sqrt", class_weight={0:1,1:9}, n_jobs=-1)
...
#+BEGIN_SRC python :session :results output
export_graphviz(rf.estimators_[0], out_file=dotfile)
#+END_SRC

#+RESULTS:
: Traceback (most recent call last):
:   File "<stdin>", line 1, in <module>
: NameError: name 'dotfile' is not defined
